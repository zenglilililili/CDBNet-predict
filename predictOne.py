...
def getSliceTrain(ct_path, label_path):
...
def classifier(model1_path, model2_path, device, ct_path, label_path, save_path):
    train_img, train_label, ctv = getSliceTrain(ct_path, label_path)
...
testSlice = 'model1.pkl'
SegSlice = 'model2.pkl'
train_or_val = "test"  # True表示train
...
for i in slice_List:
    ...
    lable_path = "data/label/" + i + ".npy"

    classifier(result_path + testSlice, result_path + SegSlice, device, ct_path, lable_path, save_path)
